...
def evaluate_models_on_test_set(models, kmeans, X_train_pca, y_train, X_test, y_test, movement_names):
    """
...
def create_visualizations(X_train_pca, y_train, kmeans, X_test_pca, y_test, movement_names, 
    ...
    plt.figure(figsize=(12, 10))
    # Define marker shapes for each movement
    markers = ['o', '^', 's', 'D', 'v', 'p', '*']  # Different shapes for each movement
    # Plot each cluster with true movement shapes
    for cluster in range(kmeans.n_clusters):
    ...
    # Add legend for clusters and movements
...
def main():
    ...
    results = evaluate_models_on_test_set(models, kmeans, X_train_pca, y_train, X_test_pca, y_test, movement_names)
