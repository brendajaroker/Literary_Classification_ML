...
def create_visualizations(X_train_pca, y_train, kmeans, X_test_pca, y_test, movement_names, 
                         results, output_dir, random_state, models, feature_names):
    """
    ...
    # PCA visualization of K-Means clusters with true movements as marker shapes
    plt.figure(figsize=(12, 10))
    markers = ['o', '^', 's', 'D', 'v', 'p', '*']  # Different shapes for each movement
    for cluster in range(kmeans.n_clusters):
        cluster_mask = (kmeans.labels_ == cluster)
        for movement_idx, (movement, marker) in enumerate(zip(movement_names, markers)):
            movement_mask = (y_train == movement_idx)
            mask = cluster_mask & movement_mask
            plt.scatter(X_train_pca[mask, 0], X_train_pca[mask, 1], 
                        label=None if cluster > 0 or movement_idx > 0 else f'Cluster {cluster}', 
                        c=sns.color_palette()[cluster], marker=marker, alpha=0.7)
    from matplotlib.lines import Line2D
    cluster_legend = [Line2D([0], [0], color=sns.color_palette()[i], lw=4, label=f'Cluster {i}') 
                      for i in range(kmeans.n_clusters)]
    movement_legend = [Line2D([0], [0], marker=markers[i], color='k', linestyle='None', 
                              label=movement_names[i], markersize=10) 
                       for i in range(len(movement_names))]
    plt.legend(handles=cluster_legend + movement_legend, loc='best')
    plt.title('K-Means Clusters Visualized with PCA')
    plt.xlabel('PCA Component 1')
    plt.ylabel('PCA Component 2')
    plt.tight_layout()
    plt.savefig(os.path.join(viz_dir, 'kmeans_pca.png'), dpi=300)

def save_results(models, kmeans, results, output_dir, scaler, pca, feature_names, movement_names):
    """
    ...
    for name, model_dict in models.items():
        model_path = os.path.join(models_dir, f'{name}_{timestamp}.joblib')
        joblib.dump(model_dict['model'], model_path)
        print(f"Saved {name} model to {model_path}")
    
    kmeans_path = os.path.join(models_dir, f'kmeans_{timestamp}.joblib')
    joblib.dump(kmeans, kmeans_path)
    print(f"Saved KMeans model to {kmeans_path}")
    
    joblib.dump(scaler, os.path.join(models_dir, f'scaler_{timestamp}.joblib'))
    joblib.dump(pca, os.path.join(models_dir, f'pca_{timestamp}.joblib'))
    ...
    pd.DataFrame(results_summary['model_accuracies'], index=[0]).to_csv(
        os.path.join(output_dir, f'accuracy_results_{timestamp}.csv'), index=False
    )
    
    for name, result in results.items():
        report_df = pd.DataFrame(result['report']).transpose()
        report_df.to_csv(os.path.join(output_dir, f'{name}_report_{timestamp}.csv'))
...
def main():
    ...
    save_results(models, kmeans, results, args.output_dir, scaler, pca, feature_names, movement_names)
